...
if [ "${CONDA_BUILD:-0}" = "1" ]; then
  CXXFLAGS_USED="@CXXFLAGS@ -I${BUILD_PREFIX}/lib/clang/${MAJOR_VERSION}/include -I${PREFIX}/include/c++/v1 -isystem ${PREFIX}/include -fdebug-prefix-map=${SRC_DIR}=/usr/local/src/conda/${PKG_NAME}-${PKG_VERSION} -fdebug-prefix-map=${PREFIX}=/usr/local/src/conda-prefix"
  DEBUG_CXXFLAGS_USED="@CXXFLAGS@ @DEBUG_CXXFLAGS@ -I${BUILD_PREFIX}/lib/clang/${MAJOR_VERSION}/include -I${PREFIX}/include/c++/v1 -isystem ${PREFIX}/include -fdebug-prefix-map=${SRC_DIR}=/usr/local/src/conda/${PKG_NAME}-${PKG_VERSION} -fdebug-prefix-map=${PREFIX}=/usr/local/src/conda-prefix"
else
