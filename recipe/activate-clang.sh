...
if [[ "${MACOSX_DEPLOYMENT_TARGET:-0}" != "0" && "$CPPFLAGS_USED" != *"macosx-version-min"* ]]; then
...
if [ "${CONDA_BUILD_SYSROOT_TEMP}" = "0" ]; then
  if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    # For cross-compilation, CONDA_BUILD_SYSROOT should be explicitly set
...
if [ "${MACOSX_DEPLOYMENT_TARGET:-0}" != "0" ]; then
  if [[ "${_CMAKE_ARGS}" != *"CMAKE_OSX_DEPLOYMENT_TARGET"* ]]; then
...
if [[ "$OSTYPE" == "darwin"* ]] && [ -n "${CONDA_BUILD_SYSROOT_TEMP}" ]; then
  if [[ "${_CMAKE_ARGS}" != *"CMAKE_OSX_SYSROOT"* ]]; then
...
_tc_activation \
  ...
  "SDKROOT,${SDKROOT:-${CONDA_BUILD_SYSROOT_TEMP}}" \
